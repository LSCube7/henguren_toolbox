...
内容页面使用带语言前缀的静态路由，例如 `/zh-CN/settings` 和 `/en-US/settings`。直接打开带前缀的地址只影响本次访问，不会覆盖已保存的语言偏好；用户在设置页主动切换语言时，才会保存偏好并切换当前地址。旧的无前缀地址会在浏览器端根据已保存语言、浏览器语言和默认语言依次跳转，查询参数与 hash 会保留。
...
开发者模式在“个人设置 → 高级设置”中启用。启用后，侧边导航 rail 底部会出现“开发者选项”入口；语言键名审核和自定义 R2 同步源配置集中在带语言前缀的 `/zh-CN/developer` 或 `/en-US/developer` 页面（旧 `/developer` 地址仍兼容）。
