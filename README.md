...
开发者模式在“个人设置 → 高级设置”中启用。启用后，侧边导航 rail 底部会出现“开发者选项”入口；语言键名审核和自定义 R2 同步源配置集中在 `/developer` 页面。

## 首次使用引导

首次使用引导会先要求用户选择登录或明确跳过。登录成功或检测到已有会话后，引导会检查账户中是否存在云端设置快照；如果存在，用户必须明确选择“使用云端设置”或“保留本机设置”，不会自动覆盖本地或云端数据。完成登录与云端设置检查后，再配置学习阶段、主题外观和后续同步偏好。
